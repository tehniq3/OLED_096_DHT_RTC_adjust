...
//if ((now - prev > 100*interval)) { //Determine whether to start a time and screen update
//if (t.sec <1) float te = dht.readTemperature();
float te = dht.readTemperature();
int t1 = 10*te;
//dtostrf(t, 4, 1, tempF);

//  temperature = (temperature * 1.8) + 32.0; // Convert Celsius to Fahrenheit
//  dtostrf(t, 4, 1, tempF); //Convert temperature to string for display

//if (t.sec <1) h = dht.readHumidity();
h = dht.readHumidity();

display.setCursor(1,15); //Set cursor for temperature display
  display.print("!"); 
delay(250);
//}
*/
